---
{"dg-publish":true,"permalink":"/knowledge/cryptography/verifiable-computing/proof/lpcp/linear-probabilistically-checkable-proofs/","dg-note-properties":{}}
---
...
> Let $\mathbb{F}$ be a finite field and $\mathcal{CS} = (n, N_g, N_w, \{a_i, b_i, c_i\}_{i \in [N_g]})$ an [[knowledge/cryptography/verifiable computing/relations/r1cs/Rank-1 Constraint Statisfiability#Basic Definition\|R1CS]] over $\mathbb{F}$. A **$k$-query input-independent linear PCP** for $\mathcal{CS}$ with query length $m$ is a tuple of algorithms $\Pi_{\mathsf{LPCP}} = (\mathsf{Query}, \mathsf{Prove}, \mathsf{Verify})$:
